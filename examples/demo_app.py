...
if viz_type == "H3 choropleth":
    ...
    st.caption("Points binned into H3 hexagons. Cell colour encodes aggregated weight.")
    agg = shv.h3_map(
        df,
        lat="lat",
        lon="lon",
    ...
    resolution = 7
    df["h3_index"] = [
        _h3.latlng_to_cell(r.lat, r.lon, resolution) for r in df.itertuples()
    ]
    pre_indexed = (
    ...
    st.subheader("A5 map")
    shv.a5_map(df, lat="lat", lon="lon", weight=weight_col, key="A5 map")
